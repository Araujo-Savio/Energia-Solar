...
namespace SolarEnergy.Services
{
    public class SimulationExportService : ISimulationExportService
    {
        public string GenerateCsv(UserSimulationResultViewModel result)
        {
            ...
            sb.AppendLine($"Prazo de payback (anos);{(result.PaybackYears?.ToString("N2", culture) ?? "Não recupera")}");
            ...
            sb.AppendLine($"Desconto aplicado na conta;{(result.RentalDiscountRate * 100).ToString("N2", culture)}%");
            ...
            sb.AppendLine($"Horizonte analisado;{result.Input.HorizonYears.ToString(culture)} anos");
